...
namespace MyPersonalWebsite.Services
{
    public class BrevoEmailService
    {
        ...
        // ============================================================
        // 3. 管理员审核邮件（含通过/拒绝按钮 + 头像）
        // ============================================================

        public async Task SendAdminVerificationRequestAsync(string username, string email, int userId, string? avatarUrl)
        {
            ...
            var avatarHtml = string.IsNullOrEmpty(avatarUrl) 
                ? "<p style='color:#888;font-size:14px;'>未上传头像</p>" 
                : $"<img src='{baseUrl}{avatarUrl}' style='width:80px;height:80px;border-radius:50%;object-fit:cover;border:2px solid #0D6EFD;' />";

            var html = $@"
                <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 10px;'>
                    <h2 style='color: #0D6EFD;'>📧 新用户审核请求</h2>
                    <p>您好，管理员！</p>
                    <p>有新用户完成邮箱验证，等待审核：</p>
            ...
            await SendEmailAsync(_adminEmail, "📧 新用户审核请求", html);
        }

        // ============================================================
        // 4. 用户操作通知（审核通过/拒绝/封禁/解封/删除/激活）
        // ============================================================
        ...
        public async Task SendUserActionNotificationAsync(string toEmail, string username, string actionType, string reason, string note)
        {
            ...
            var color = actionType == "approve" ? "#28a745" : 
                        actionType == "reject" ? "#dc3545" : 
        ...
        public async Task SendAdminNewMessageNotificationAsync(string visitorName, string content, int messageId)
        {
            var html = $@"
                <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 10px;'>
                    ...
                    <a href='https://chris-hopper.org/Admin/Messages' style='display: inline-block; padding: 10px 20px; background-color: #0D6EFD; color: white; text-decoration: none; border-radius: 5px;'>点击审核</a>
                    <hr style='border: none; border-top: 1px solid #eee;'>
                    <p style='color: #aaa; font-size: 12px;'>此邮件由系统自动发送，请勿直接回复。</p>
                </div>
            ";
