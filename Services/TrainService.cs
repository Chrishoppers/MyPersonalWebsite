    ...
    public class TrainService
    {
        ...
        private async Task<List<TrainStopDetail>> GetMockTrainStopsWithDetailAsync(string trainCode)
        {
            ...
            for (int i = 0; i < routeNames.Count; i++)
            {
                ...
                // ⭐ 修复：使用 if-else 替代 switch
                int colorIdx = i % 4;
                string landmarkColor = colorIdx == 0 ? "🟡 黄色（8节正编）" :
                                       colorIdx == 1 ? "🟢 绿色（16节正编）" :
                                       colorIdx == 2 ? "🟣 紫色（8节反编）" :
                                       "🔵 蓝色（16节反编）";
        ...
        private List<string> GetRouteData(string trainCode)
        {
            var routes = new Dictionary<string, List<string>>
            {
                ...
                { "G15", new List<string> { "北京南", "天津南", "德州东", "济南西", "徐州东", "南京南", "上海" } },
                { "G16", new List<string> { "上海", "南京南", "徐州东", "济南西", "德州东", "天津南", "北京南" } },
